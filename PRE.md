...
After that, run `pip install -r requirements.txt` under `PRE/` to install a few more packages required by [CLIP](https://github.com/openai/CLIP) (this should be done when `dassl` is activated). Then, you are ready to go.
...
Below we provide an example on how to evaluate the model on DTD dataset.

```bash
bash base2new_train.sh dtd
bash base2new_test.sh dtd
```
...
```bash
python parse_test_res.py output/base2new/train_base/dtd/shots_16/PRE_LSTM1_True_False/vit_b16_ep50_ctxv1
```

To get the average performance on the new classes, run

```bash
python parse_test_res.py output/base2new/test_new/dtd/shots_16/PRE_LSTM1_True_False/vit_b16_ep50_ctxv1 --test-log
```
